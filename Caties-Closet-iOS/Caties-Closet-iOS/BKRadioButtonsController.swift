//
//  BKRadioButtonsController.swift
//  Caties-Closet-iOS
//
...
protocol BKRadioButtonsControllerDelegate {
    func selectedButton(sender: BKRadioButton);
}
class BKRadioButtonsController: NSObject {
    var delegate : BKRadioButtonsControllerDelegate?
    var radioButtons: [BKRadioButton]!
    ...
    init(radioButtons : [BKRadioButton]) {
        super.init()
        
        self.radioButtons = radioButtons
        for (index, button) in self.radioButtons.enumerated() {
            button.tag = index
            button.addTarget(self, action: #selector(BKRadioButtonsController.select(sender:)), for: .touchUpInside)
            //button.setTitleColor(UIColor .black, for: UIControlState.normal)
            //button.setTitleColor(UIColor .gray, for: UIControlState.highlighted)
        }
        redrawButtons()
    }
    @objc func select(sender: BKRadioButton ){
